...
const promptExamples = {
  HeroSectionComponent:
    "Create a hero section for a travel agency in turquoise",
  DashboardComponent: "Create a dashboard component for a tech company, black",
  FeaturesSectionComponent:
    "Create a features section for a food delivery startup, orange",
  ModalComponent: "Create a modal for confirmation of deleting account, red",
  VerticalCardComponent:
    "Create a grid of vertical cards for travel destinations, black",
  HorizontalCardComponent: "Create a grid of horizontal cards for food items",
  TestimonialComponent:
    "Generate a testimonial component for a fitness company, blue",
};
    ...
    <Transition.Root show={showModalForm} as={Fragment}>
      ...
      >
        <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
          ...
          >
            <div className="inline-block align-bottom bg-[#374151] rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-screen-sm sm:w-full sm:p-6">
              <div className="">
                <div className="max-w-2xl mx-auto py-2 px-6 lg:max-w-7xl">
                      ...
                      <div>
                        <div className="pb-2">
                          <span className="flex flex-row">
                            ...
                            <span className="">
                              <h2 className="text-xl font-bold text-slate-300 px-2">
                                Create your component
                              </h2>
                              <p className="text-sm py-2 px-2 font-medium text-slate-300">
                                Enter a prompt describing the component you want
                                TailMagic to generate. Make sure to include your
                                preferred colour scheme and the theme of the
                                component (eg. e-commerce, food, finance) for
                                best results.
                              </p>
                            </span>
                          </span>
                  ...
                  <div className="mt-2">
                    <input
                      ...
                      className="placeholder:text-slate-300 block w-full border-gray-300 rounded-md shadow-sm focus:ring-teal-300 focus:border-teal-300 sm:text-sm"
                    />
                  </div>
                </div> */}
                          <div className="col-span-2">
                            <label className="block text-sm font-medium text-slate-300 py-1">
                                        ...
                                        <Listbox.Option
                                          ...
                                          className={({ active }) =>
                                            `relative cursor-default select-none py-2 pl-10 pr-4 ${
                                              active
                                                ? "bg-teal-100 text-black font-semibold"
                                                : "text-black"
                                            }`
                                          }
                          ...
                          <div className="sm:col-span-2 pb-2">
                            ...
                            <div className="mt-2">
                              <textarea
                                name="prompt"
                                id="prompt"
                                className="placeholder:text-black block w-full px-2 h-24 border-2 border-gray-200 rounded-md shadow-sm focus:ring-teal-300 focus:border-teal-300 sm:text-sm"
                                placeholder={`Example:  ${
                                  promptExamples[selectedComponent.value]
                                }`}
                                onChange={(e) => {
                  ...
                  <>
                {
                  availableComponents.map((availableComponent)=>{
                    <option value={availableComponent.value}>{availableComponent.label}</option>
                          ...
                          <div className="sm:col-span-2">
                            <fieldset className="border-t border-slate-300">
                              <legend className="mx-auto px-4 block text-sm text-center font-medium text-slate-300">
                                OR
                              </legend>
                            </fieldset>

                            <label className="block text-sm font-medium text-slate-300 pt-4 pb-2 ">
                              Copy one of the below prompts
                            </label>
                            <div className="mt-2 grid grid-rows-2 gap-y-4">
                              <div className="bg-slate-100 rounded-sm text-sm text-center text-slate-500 font-semibold px-2 py-3 w-full">
                                Create a hero section for a food delivery
                                website, orange in colour
                              </div>
                              <div className="bg-slate-100 rounded-sm text-sm text-center text-slate-500 font-semibold px-2 py-3 w-full">
                                Create a feature section for an edtech startup
                              </div>
                              <div className="bg-slate-100 rounded-sm text-sm text-center text-slate-500 font-semibold px-2 py-3 w-full">
                                Create a dashboard component for a financial
                                company, teal
                              </div>
                            </div>
                          </div>
                    ...
                    <div className="mt-8 sm:grid sm:grid-cols-2 sm:gap-3 sm:grid-flow-row-dense">
                      ...
                      <button
                        type="button"
                        className="w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-slate-500 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-300 sm:mt-0 sm:col-start-1 sm:text-sm"
